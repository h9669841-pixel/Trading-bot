...
# --- 📊 DİNAMİK EN YÜKSEK HACİMLİ 100 COIN BULUCU ---
def ilk_100_hacimli_coin_bul():
    try:
        # Futures 24 saatlik istatistikleri çek
        ticker_url = "https://fapi.binance.com/fapi/v1/ticker/24hr"
        response = requests.get(ticker_url, timeout=15).json()
        
        # Sadece USDT paritelerini filtrele ve hacme göre büyükten küçüğe sırala
        ...
        # İlk 100 paritenin sembollerini küçük harfli olacak şekilde listeye al
        en_hacimli_100 = [x["symbol"].lower() for x in sorted_by_volume[:100]]
        return en_hacimli_100
    except Exception as e:
        print(f"❌ Hacim listesi API'den alınamadı: {e}")
        return []
...
def trend_tarama_dongusu():
    while True:
        try:
            ...
            # 🖥️ LOG EKRANI BİLGİLENDİRMESİ (Her tarama başında konsola yazılır)
            su_an = datetime.now().strftime('%H:%M:%S')
            print(f"🔄 [{su_an}] En yüksek hacimli ilk {len(SYMBOLS)} coin taranıyor... Bot aktif ve çalışıyor.")
            ...
            traceback.print_exc()
        time.sleep(2.5)  # ⏱️ 100 coini çok sıkıştırmamak ve API limitlerine takılmamak için döngü arası hafif genişletildi.

if __name__ == "__main__":
    set_telegram_menu_commands()
    
    print("⏳ API üzerinden en yüksek hacimli ilk 100 vadeli işlem paritesi çekiliyor...")
    aday_listesi = ilk_100_hacimli_coin_bul()
    
    if not aday_listesi:
        print("❌ Hata: İlk 100 koin listesi alınamadı. Bot başlatılamıyor.")
        exit()
        
    for coin in aday_listesi:
        kontrollu_coin_ekle(coin)
        
    print(f"🚀 Başarılı! En yüksek hacimli {len(SYMBOLS)} parite veri hattına bağlandı.")
    ...
    time.sleep(3.0)
    telegram_bildir(f"🤖 <b>Dinamik İlk 100 Hacim Avcısı Canlıda!</b>\n📊 Taranan Parite Sayısı: {len(SYMBOLS)}\n💰 Poz Büyüklüğü: 20 USDT\n🛑 Maks Eşzamanlı Poz: {config.MAX_ACIK_POZISYON}\nTarama döngüsü başlatıldı.")
